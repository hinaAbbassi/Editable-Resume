...
function addNewintr(){
    let newNode = document.createElement("textarea");
    newNode.classList.add("form-control")
    newNode.classList.add("intrfeild")
    newNode.classList.add("col")
    newNode.style.marginTop = "10px";
    newNode.style.marginBottom = "5px";
    newNode.style.padding = "10px";
    ...
    let intraddbtn = document.getElementById("intraddbtn")

    intr.insertBefore(newNode,intraddbtn);
}
...
//    internship
let intrfeild = document.getElementsByClassName("intrfeild");
let intr = ""
for(let itr of intrfeild){
    intr = intr + `<li>${itr.value}</li>`
}
document.getElementById("internshipT").innerHTML = intr;
//    profeild
...
// images
  ...
  function editCv() {
    ...
    // Reset styles (if required)
    const form = document.getElementById("cv-form");
    form.style.margin = "0 auto"; // Center the form
    form.style.width = "80%"; // Adjust width
    form.style.flexDirection = "column"; // Ensure proper layout
}
